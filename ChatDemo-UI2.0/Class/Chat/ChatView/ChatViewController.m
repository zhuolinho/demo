...
- (void)viewDidLoad
{
    ...
    _isChatGroup = NO;
    _chatToolBar.backgroundImage = nil;

    _chatToolBar.backgroundColor = [[UIColor alloc] initWithHue:0 saturation:0 brightness:0.93 alpha:1];
    //根据接收者的username获取当前会话的管理者
    ...
    if (_buyCourseRightNow) {
        NSString *str = @"老师，赶紧来上课吧~";
        NSString *username = _chatter;
    ...
    [tap addTarget:self action:@selector(keyBoardHidden)];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    _chatToolBar.backgroundImage = nil;
    _chatToolBar.backgroundColor = [[UIColor alloc]initWithHue:0 saturation:0 brightness:0.9 alpha:1];
...
//        _tableView.backgroundColor = [UIColor lightGrayColor];
        _tableView.backgroundColor = [[UIColor alloc] initWithHue:0 saturation:0 brightness:0.93 alpha:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (model.isSender) {
                model.headImageURL = [[NSURL alloc] initWithString:_myHeadUrl];
            }
            else {
...
//    [self.navigationController pushViewController:locationController animated:YES];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"暂不支持" message:@"" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alert show];
}
